...
export default function Step4({ formData, setFormData }) {
  ...
  const [checkboxStates1, handleCheckboxChange1] =
    useCheckboxStates(Basics);
...
const boxdivStyle ={
  padding:isMobile?"0px":isTablet?"0px":"" ,marginTop:isMobile?"2%":"" ,    marginBottom: isMobile?"":islaptop_isTablet?"5%":"2%" , 
}
...
useEffect(() => {
  ...
  const selectedBasics = Object.keys(checkboxStates1).filter((key) => checkboxStates1[key]);
...
useEffect(() => {
  const selectedFoodBeverage = Object.keys(checkboxStates2).filter((key) => checkboxStates2[key]);
  setFormData((prevData) => ({
    ...prevData,
    amenities: {
      ...prevData.amenities,
...
useEffect(() => {
  const selectedElectronics = Object.keys(checkboxStates3).filter((key) => checkboxStates3[key]);
  setFormData((prevData) => ({
    ...prevData,
    amenities: {
      ...prevData.amenities,
...
useEffect(() => {
  const selectedSea_sports = Object.keys(checkboxStates4).filter((key) => checkboxStates4[key]);
  setFormData((prevData) => ({
    ...prevData,
    amenities: {
      ...prevData.amenities,
...
useEffect(() => {
  const selectedMega_yaghts = Object.keys(checkboxStates5).filter((key) => checkboxStates5[key]);
  setFormData((prevData) => ({
    ...prevData,
    amenities: {
      ...prevData.amenities,
...
useEffect(() => {
  const selectedYaght_equipment = Object.keys(checkboxStates6).filter((key) => checkboxStates6[key]);
  setFormData((prevData) => ({
    ...prevData,
    amenities: {
      ...prevData.amenities,
  ...
  return (
    <div className={`container${islaptop_isTablet ? '-fluid' : ''}`} style={container}>
      ...
      >
        <div className="col-12 col-md-12  col-lg-10 " style={{ marginTop: "3%",padding:isTablet?"0px":"" }}>
         
          <img src={isMobile?Step3mb:step3} width="100%" alt="" />
        </div>
      </div>
      
        <div class="row justify-content-center" style={{padding:isMobile?"":isTablet?"2% 2% 5% 5%":"" ,marginLeft:isMobile?"":isTablet?"":"4%"}}>
